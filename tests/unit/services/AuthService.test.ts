...
import type { IUserRepository, ISessionRepository, SessionWithSecret } from "@repositories/interfaces";
...
describe("AuthService Unit Test (Strict Types)", () => {
    ...
    describe("verifyToken()", () => {
        ...
        test("should return null when secret does not match an existing session", async () => {
            const result = await authService.verifyToken(`${mockSession.id}.000000000000000000000000000000`);
            expect(result).toBeNull();
